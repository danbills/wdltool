...
object GraphPrint {
  ...
  case class WorkflowDigraph(workflowName: String, digraph: NodesAndLinks)
  case class NodesAndLinks(nodes: Set[String], links: Set[String])
  implicit val scopesAndLinksMonoid = new Monoid[NodesAndLinks] {
  ...
  sealed trait Link{
    def linkString: String
  }

  case class GraphLink(linkString: String, scopePath: List[Scope])
  ...
  private def defaultFilter: WdlGraphNode => Boolean = _ => true

  val clusterCount: AtomicInteger = new AtomicInteger(0)


  private def listAllGraphNodes(namespace: Scope): NodesAndLinks = {

    val callsAndDeclarations: Set[WdlGraphNode] = (namespace.children collect {
      case w: WdlGraphNode if isCallOrDeclaration(w) => w
    }).toSet

    val subGraphs: Set[WdlGraphNode] = (namespace.children collect {
      case s: Scatter => s
      case i: If => i
    }).toSet
